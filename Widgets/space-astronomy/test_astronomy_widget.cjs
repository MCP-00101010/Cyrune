...
function loadAstronomyWidgets() {
  ...
  for (const filename of [
    ...
    '../Widgets/space-astronomy/astronomy-events.js',
    '../Widgets/core/widgets.js',
    '../Widgets/space-astronomy/astronomy-widget.js'
  ]) {
...
test('astronomy assets are local, licensed and loaded before widget code', () => {
  ...
  assert.ok(index.indexOf('vendor/astronomy-engine/astronomy.browser.min.js') < index.indexOf('../Widgets/space-astronomy/astronomy-widget.js'));
  assert.ok(index.indexOf('../Widgets/space-astronomy/astronomy-events.js') < index.indexOf('../Widgets/space-astronomy/astronomy-widget.js'));
});

test('astronomy widget exposes location, horizon and section controls with responsive styling', () => {
  const source = fs.readFileSync(path.join(__dirname, 'astronomy-widget.js'), 'utf8');
  const styles = fs.readFileSync(path.join(__dirname, 'astronomy-widget.css'), 'utf8');
