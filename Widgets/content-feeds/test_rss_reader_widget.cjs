...
const rssWidgetSource = fs.readFileSync(path.join(__dirname, 'rss-reader-widget.js'), 'utf8');
...
test('RSS UI exposes combined, starred and feed tabs with local read state', () => {
  const widgets = rssWidgetSource;
  const styles = fs.readFileSync(path.join(__dirname, 'rss-reader-widget.css'), 'utf8');
  ...
  assert.match(styles, /\.widget-rss-articles\s*\{[^}]*overflow-anchor:\s*none/s);
  assert.match(styles, /\.widget-rss-articles\s*\{[^}]*scrollbar-gutter:\s*stable/s);
  assert.match(rssSource, /article\.dataset\.articleId = item\.favoriteId/);
  assert.match(rssSource, /image\.addEventListener\('load', attachLoadedImage, \{ once: true \}\)/);
  assert.match(rssSource, /image\.decoding = 'async'/);
  assert.doesNotMatch(rssSource, /image\.loading = 'lazy'/);
  assert.match(rssSource, /if \(!article\.isConnected \|\| image\.parentNode \|\| image\.naturalWidth <= 0\) return/);
  assert.match(rssSource, /article\.insertBefore\(image, content\)/);
  assert.doesNotMatch(rssSource, /widget-rss-image-frame/);
  assert.match(rssSource, /const anchorOffset = anchor \? anchor\.offsetTop - previousScrollTop : 0/);
  assert.match(rssSource, /_setWidgetRefresher\(widget\.id, context,[\s\S]*?renderTabs\(\);[\s\S]*?renderArticles\(\)/);
  ...
  assert.match(styles, /\.widget-rss-image\s*\{[^}]*flex:\s*0 0 92px;[^}]*width:\s*92px;[^}]*height:\s*70px/s);
  assert.doesNotMatch(styles, /widget-rss-image-frame/);
