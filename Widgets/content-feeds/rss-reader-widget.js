...
WIDGET_REGISTRY['rssReader'] = {
  ...
  render(widget, el, context) {
    ...
    const renderArticles = ({ preserveAnchor = true } = {}) => {
      ...
      const previousFeedId = articles.dataset.feedId || '';
      const previousScrollTop = articles.scrollTop;
      const sameFeed = previousFeedId === currentView.activeFeedId;
      const anchor = preserveAnchor && sameFeed
        ? [...articles.children].find(child => child.classList.contains('widget-rss-article')
          && child.offsetTop + child.offsetHeight > previousScrollTop)
        : null;
      const anchorId = anchor?.dataset.articleId || '';
      const anchorOffset = anchor ? anchor.offsetTop - previousScrollTop : 0;
      articles.innerHTML = '';
      articles.dataset.feedId = currentView.activeFeedId;
      ...
      items.forEach(item => {
        ...
        article.dataset.articleId = item.favoriteId;
        let image = null;
        ...
        if (image) {
          const attachLoadedImage = () => {
            if (!article.isConnected || image.parentNode || image.naturalWidth <= 0) return;
            article.insertBefore(image, content);
          };
          image.addEventListener('load', attachLoadedImage, { once: true });
          if (image.complete) attachLoadedImage();
        }
      });
      const restoredAnchor = anchorId
        ? [...articles.children].find(child => child.dataset.articleId === anchorId)
        : null;
      articles.scrollTop = restoredAnchor
        ? Math.max(0, restoredAnchor.offsetTop - anchorOffset)
        : (preserveAnchor && sameFeed
          ? previousScrollTop
          : (currentView.articleScroll[currentView.activeFeedId] || 0));
      renderStatus();
    };
    ...
    search.addEventListener('input', () => {
      ...
      renderArticles({ preserveAnchor: false });
    });
