...
test('SDK files are ordered and the example manifest covers the contract surface', () => {
  ...
  assert.ok(html.indexOf('../Widgets/core/widget-sdk.js') < html.indexOf('calendar-widget.js'));
  const manifest = JSON.parse(fs.readFileSync(path.join(__dirname, 'sdk', 'manifest.example.json'), 'utf8'));
...
test('widget documentation and SDK rules require meaningful widget UI state to survive reloads locally', () => {
  ...
  const readme = fs.readFileSync(path.join(__dirname, 'sdk', 'README.md'), 'utf8');
...
test('widgets can move through tab Inboxes with shared drag, context-menu, and runtime boundaries', () => {
  ...
  const widgetsSource = fs.readFileSync(path.join(__dirname, 'widgets.js'), 'utf8');
...
test('substantial built-ins live in ordered standalone script and style modules', () => {
  const html = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
  const legacy = fs.readFileSync(path.join(__dirname, 'widgets.js'), 'utf8');
  const moduleIds = [
    ['nasa-apod', 'nasaApod'],
    ['weather', 'weather'],
  ...
  const widgetAssetPath = (filename, extension) => {
    const match = html.match(new RegExp(`(?:src|href)="([^"]*${filename}-widget\\.${extension})"`));
    assert.ok(match, `${filename} ${extension} is loaded by Portal`);
    return match[1];
  };

  let previousScriptIndex = html.indexOf('../Widgets/core/widget-sdk.js');
  for (const [filename, widgetId] of moduleIds) {
    const scriptPath = widgetAssetPath(filename, 'js');
    const stylePath = widgetAssetPath(filename, 'css');
    const source = fs.readFileSync(path.resolve(root, scriptPath), 'utf8');
  ...
  const standaloneWidgetFiles = [...html.matchAll(/<script[^>]+src="([^"]+-widget\.js)"/g)]
    .map(match => match[1]);
