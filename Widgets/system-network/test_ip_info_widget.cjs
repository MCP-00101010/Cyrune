...
const ipInfoSource = fs.readFileSync(path.join(__dirname, 'ip-info-widget.js'), 'utf8');
...
test('the pinned Cloudflare browser engine loads locally before widget code', () => {
  ...
  assert.ok(html.indexOf('vendor/cloudflare-speedtest/speedtest.js') < html.indexOf('ip-info-widget.js'));
...
test('IP Info UI exposes flag, address, change state, privacy controls, and attribution', () => {
  const source = ipInfoSource;
  const styles = fs.readFileSync(path.join(__dirname, 'ip-info-widget.css'), 'utf8');
