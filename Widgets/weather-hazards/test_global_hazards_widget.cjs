...
const source = fs.readFileSync(path.join(__dirname, 'global-hazards-widget.js'), 'utf8');
...
test('hazard assets load after MapLibre and SDK and expose responsive map/list styling', () => {
  ...
  const css = fs.readFileSync(path.join(__dirname, 'global-hazards-widget.css'), 'utf8');
  assert.ok(html.indexOf('vendor/maplibre-gl/maplibre-gl.js') < html.indexOf('../Widgets/weather-hazards/global-hazards-widget.js'));
  assert.ok(html.indexOf('../Widgets/core/widget-sdk.js') < html.indexOf('../Widgets/weather-hazards/global-hazards-widget.js'));
  assert.match(html, /\.\.\/Widgets\/weather-hazards\/global-hazards-widget\.css/);
