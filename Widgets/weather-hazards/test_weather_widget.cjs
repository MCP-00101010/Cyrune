...
function loadWidgets(fetchImpl = async () => ({ ok: true, json: async () => ({}) })) {
  ...
  const networkFilename = path.join(__dirname, '..', 'core', 'widget-network.js');
  ...
  for (const moduleName of ['weather-widget.js', 'weather-map-widget.js']) {
    const moduleFilename = path.join(__dirname, moduleName);
...
test('basic Weather exposes a reload action beside widget settings', () => {
  const widgets = fs.readFileSync(path.join(__dirname, '..', 'core', 'widgets.js'), 'utf8');
  const weather = fs.readFileSync(path.join(__dirname, 'weather-widget.js'), 'utf8');
  const styles = fs.readFileSync(path.join(__dirname, 'weather-widget.css'), 'utf8');
...
test('both Weather widgets force refresh on first render and each new hour', () => {
  ...
  const mapSource = fs.readFileSync(path.join(__dirname, 'weather-map-widget.js'), 'utf8');
...
test('weather map assets are pinned locally and unchanged instances survive column rerenders', () => {
  ...
  const widgets = fs.readFileSync(path.join(__dirname, 'weather-map-widget.js'), 'utf8');
  const framework = fs.readFileSync(path.join(__dirname, '..', 'core', 'widgets.js'), 'utf8');
