...
const calendarSource = fs.readFileSync(path.join(__dirname, 'calendar-widget.js'), 'utf8');
...
test('calendar assets expose secure settings and responsive agenda/month views', () => {
  assert.match(html, /\.\.\/Widgets\/personal-productivity\/calendar-widget\.css/);
  assert.match(html, /\.\.\/Widgets\/personal-productivity\/calendar-widget\.js/);
