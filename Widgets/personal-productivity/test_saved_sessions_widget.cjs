...
test('Saved Sessions assets load after the SDK and cover favicons and compact layouts', () => {
  ...
  const css = fs.readFileSync(path.join(__dirname, 'saved-sessions-widget.css'), 'utf8');
  assert.ok(html.indexOf('../Widgets/core/widget-sdk.js') < html.indexOf('../Widgets/personal-productivity/saved-sessions-widget.js'));
  assert.match(html, /\.\.\/Widgets\/personal-productivity\/saved-sessions-widget\.css/);
