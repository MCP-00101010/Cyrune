...
const trackerSource = fs.readFileSync(path.join(__dirname, 'nexus-mods-tracker-widget.js'), 'utf8');
const trackerStyles = fs.readFileSync(path.join(__dirname, 'nexus-mods-tracker-widget.css'), 'utf8');
...
test('Nexus tracker source is read-only and declares conservative provider boundaries', () => {
  const sdk = fs.readFileSync(path.join(root, '..', 'Widgets', 'core', 'widget-sdk.js'), 'utf8');
