...
const widgetSource = fs.readFileSync(path.join(__dirname, 'translator-widget.js'), 'utf8');
const workerSource = fs.readFileSync(path.join(__dirname, 'translator-worker.js'), 'utf8');
...
test('worker, relay, styles, and document wiring preserve the offline boundary', () => {
  ...
  const css = fs.readFileSync(path.join(__dirname, 'translator-widget.css'), 'utf8');
  ...
  assert.ok(html.indexOf('widget-sdk.js') < html.indexOf('translator-widget.js'));
  assert.ok(html.indexOf('translator-widget.js') < html.indexOf('source/command-palette.js'));
  assert.match(html, /translator-widget\.css/);
  ...
  assert.match(widgetSource, /\.\.\/Widgets\/utilities\/translator-worker\.js/);
  assert.doesNotMatch(widgetSource, /new Worker\(new URL/);
