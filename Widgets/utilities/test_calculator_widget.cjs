...
const source = fs.readFileSync(path.join(__dirname, 'calculator-widget.js'), 'utf8');
...
test('calculator assets load in SDK order and include compact responsive styling', () => {
  ...
  const css = fs.readFileSync(path.join(__dirname, 'calculator-widget.css'), 'utf8');
  assert.ok(html.indexOf('widget-sdk.js') < html.indexOf('calculator-widget.js'));
  assert.ok(html.indexOf('calculator-widget.js') < html.indexOf('source/command-palette.js'));
  assert.match(html, /calculator-widget\.css/);
